...
class CPixelRenderer {
 ...
 public:
  ...
  void getForeground(CRGBA &fg) const { fg = pen_.getColor(); }
  ...
  uint getLineWidth() const { return pen_.getWidth(); }
  virtual void setLineWidth(uint width);

  const CILineDash &getLineDash() const { return pen_.getLineDash(); }
  virtual void setLineDash(const CILineDash &dash);

  CLineCapType getLineCap() const { return pen_.getLineCap(); }
  virtual void setLineCap(CLineCapType cap);

  CLineJoinType getLineJoin() const { return pen_.getLineJoin(); }
